...
def get_java_home():
    ...
    if env_home:
        ...
        if os.path.exists(env_home):
            ...
            configure_error(
                "Path " + env_home + " indicated by JAVA_HOME does not exist."
            )
...
def get_java_lib():
    ...
    if not os.path.exists(lib):
        configure_error(
            "Lib folder should be at '{0}' but doesn't exist. "
    ...
    def run(self):
        ...
        if is_osx():
            ...
            if target:
                ...
                if (
                    target.split(".")[:2]
                    != result.stdout.decode("utf-8").split(".")[:2]
                ):
                    warning(
...
class build_java(Command):
    ...
    def build(self, *jclasses):
        jep = [
            x
            for x in list(*jclasses)
            if not x.startswith("src{0}test{0}java{0}".format(os.sep))
        ]
        tests = [
            x
            for x in list(*jclasses)
            if x.startswith("src{0}test{0}java{0}".format(os.sep))
        ]
        self.spawn(
            [
                self.javac,
        ...
        self.spawn(
            [
                ...
                "-deprecation",
                "-d",
                build_java.testoutdir,
                "-classpath",
                "{0}{1}src".format(build_java.outdir, os.pathsep),
            ]
            + tests
        )
...
class build_jar(Command):
    ...
    user_options = [
        ("jar=", None, "use jar (default: {0}/bin/jar)".format(get_java_home())),
    ]
    ...
    def build(self):
        ...
        self.spawn(
            [
                self.jar,
                "-cf",
                "build/java/jep-{0}-sources.jar".format(self.version),
                "-C",
                "build/java/jep.src/main/java",
                "jep",
                "-C",
                "build/java",
                "META-INF",
        ...
        self.spawn(
            [
                self.jar,
                "-cfe",
                "build/java/jep-{0}.jar".format(self.version),
                "jep.Run",
                "-C",
                "build/java",
                "jep",
        ...
        self.spawn(
            [
                self.jar,
                "-cf",
                "build/java/jep-{0}-test-sources.jar".format(self.version),
                "-C",
                "build/java/jep.test.src/test/java",
                "jep",
                "-C",
                "build/java",
                "META-INF",
        ...
        self.spawn(
            [
                self.jar,
                "-cfe",
                "build/java/jep-{0}-test.jar".format(self.version),
                "test.jep.Test",
                "-C",
                "build/java/test",
                "jep",
